...
// kmem_cache contains several slabs, slabs in one kmem_cache have same object size
// slab contains several objects, objects in one slab have same size
struct kmem_cache_t *
...
kfree(void *ptr, size_t n) {
    ...
    void *kva = ROUNDDOWN(ptr, PGSIZE);
    struct slab_t *slab = (struct slab_t *) &pages[(kva-base)/PGSIZE];
    kmem_cache_free(slab->cachep, ptr);
}*/
...
kmem_int() {
    ...
    // Init sized cache 
    int i = 0, size = 16;
    for (; i < SIZED_CACHE_NUM; i++, size *= 2)
