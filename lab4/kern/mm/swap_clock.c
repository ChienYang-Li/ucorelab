...
_clock_swap_out_victim(struct mm_struct *mm, struct Page ** ptr_page, int in_tick)
{
     ...
     list_entry_t *selected = NULL, *p = head, *modified = NULL;

     do{//outer loop at most execute 2 times
		 do{
			 int accessed = GET_ACCESSED_FLAG(mm->pgdir, p);
			 int dirty = GET_DIRTY_FLAG(mm->pgdir, p);

			 if(accessed==0 && dirty==0){
				 selected = p;
				 break;
			 }
			 else if(accessed==0 && dirty){
				 if(modified==NULL)
					 modified = p;
			 }
			 else{
		 ...
		 if(modified!=NULL && selected == NULL){
			 selected = modified;
			 break;
		 }
...
_clock_check_swap(void) {
    cprintf("write Virt Page c in _clock_check_swap\n");
    *(unsigned char *)0x3000 = 0x0c;
    assert(pgfault_num==4);
    cprintf("write Virt Page a in _clock_check_swap\n");
    *(unsigned char *)0x1000 = 0x0a;
    assert(pgfault_num==4);
    cprintf("write Virt Page d in _clock_check_swap\n");
    *(unsigned char *)0x4000 = 0x0d;
    assert(pgfault_num==4);
    cprintf("write Virt Page b in _clock_check_swap\n");
    *(unsigned char *)0x2000 = 0x0b;
    assert(pgfault_num==4);
    cprintf("write Virt Page e in _clock_check_swap\n");
    ...
    *(unsigned char *)0x1000 = 0x0a;
    assert(pgfault_num==9);
    cprintf("read Virt Page b in _clock_check_swap\n");
    assert(*(unsigned char *)0x2000 == 0x0b);
    assert(pgfault_num==10);
    cprintf("read Virt Page c in _clock_check_swap\n");
    assert(*(unsigned char *)0x3000 == 0x0c);
    assert(pgfault_num==11);
    cprintf("read Virt Page a in _clock_check_swap\n");
    assert(*(unsigned char *)0x1000 == 0x0a);
    assert(pgfault_num==11);
    cprintf("read Virt Page d in _clock_check_swap\n");
