...
// virtual size is power of 2 that buddy system theoretically handle
// alloc_size is physical memory size
// buddy_segment_size is page size that used for record node information
static void
...
/*
 * buddy_segment: an array of size_t that store a complete binary tree
 * 				  which record the buddy system memory allocation state
 * buddy_alloc: an array of physical page that available to allocate
 */
//buddy_init_segment construct buddy_segment as binary tree
static void
buddy_init_segment(struct Page *base) {
    ...
    //example:virtual_size=16,alloc_size=14
    while (alloc_size > 0 && alloc_size < virtual_size) {
        virtual_size >>= 1;
        //add physical pages(size=virtual size) to the left child, and
        //process remaining physical pages
        ...
        //add finally remaining physical pages to the left child
        else {
            // Switch to left
...
buddy_alloc_pages(size_t n) {
    ...
    while (length <= buddy_segment[block] && length < BUDDY_LENGTH(block)) {
        ...
        if (BUDDY_EMPTY(block)) {                   // Split if no used at all
            ...
            block = left;
        } else if (length & buddy_segment[left]) {  // Find in left (optimize) if size equal
            block = left;
        } else if (length & buddy_segment[right]) { // Find in right (optimize) if size equal
            block = right;
